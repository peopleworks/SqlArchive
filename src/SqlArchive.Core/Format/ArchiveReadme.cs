...
public static class ArchiveReadme
{
    public static string Compose(ArchiveManifest manifest)
    {
        ...
        text.AppendLine()
            ...
            .AppendLine("line exactly as it is in the file, without its newline. Summing rather than")
            .AppendLine("concatenating is what makes it independent of the order the rows were read in, so a")
            .AppendLine("table read in parallel ranges, or read back in a different order, still adds up to")
            .AppendLine("the same value. It says that the content is the same; it does not say which row")
            .AppendLine("changed. The row count beside it is part of the check, not decoration.")
            .AppendLine()
